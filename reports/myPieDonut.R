myPieDonut <- function (data, mapping, start = getOption("PieDonut.start", 0), 
                        addPieLabel = TRUE, 
                        addDonutLabel = TRUE, 
                        showRatioDonut = TRUE, 
                        showRatioPie = TRUE, 
  ...
  if (!is.null(donuts)) {
    ...
    df3
    if (ratioByGroup) {
      df3$ratio = paste0(round(df3$Freq/df3$group*100),"%")
    }
    else {
      df3$ratio <- paste0(round(df3$ratio1*100),"%")
    }
  ...
  if (!is.null(donuts)) {
    if (explodeDonut) {
      p3 <- p + geom_arc_bar(aes_string(x0 = "x", 
